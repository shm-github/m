...
public class SrtParser implements SubtitleParser {
    ...
    public SrtObject parse(InputStream is, boolean strict) throws IOException, SubtitleParsingException {
        ...
        ArrayList<Character> arr = new ArrayList<>(Arrays.asList(nums));
        ...
        while ((textLine = br.readLine()) != null) {
            ...
            if(textLine.length() > 1 && cursorStatus == CursorStatus.NONE) {
                if (arr.contains(textLine.charAt(0)) )
                    textLine = String.valueOf(arr.indexOf(textLine.charAt(0)));
                else if (arr.contains(textLine.charAt(1)))
                    textLine = String.valueOf(arr.indexOf(textLine.charAt(1)));
